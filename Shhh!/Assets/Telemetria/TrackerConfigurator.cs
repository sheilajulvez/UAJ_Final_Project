...
public enum TelemetryTrackerEventType
{
    ProgressionTracker,
    ResourceTracker
}
...
public class TrackerConfigurator : MonoBehaviour
{
    private void Awake()
    {
        ...
        if (!File.Exists(path))
        {
            Debug.LogError("No se encontro el archivo de configuracion en: " + path);
            return;
        }
        ...
        {
            Debug.LogError("No se encontro el objeto 'trackers' en el JSON.");
        }
